    ...
    V: Clone,
{
    ...
    /// Updates the capacity of the map.
    ///
    /// # Parameters
    ///
    /// - `additional`: The number of additional slots to allocate.
    ///
    fn expand_capacity(&mut self, additional: usize) {
        // This is ensured by the calling contexts.
        debug_assert!(additional > 0);

        // Expand indices vector
        self.indices
            .resize_with(self.capacity() + additional, VecDeque::new);
    ...
    /// Suggests the new capacity of the map
    fn additional_capacity(&self) -> usize {
        // Calculate the new capacity
        let new_capacity = (self.len() as f64 / LOAD_FACTOR).ceil() as usize;
        let new_capacity = new_capacity
            .checked_next_power_of_two()
            .unwrap_or(usize::MAX);
        // Calculate and return the additional capacity
    ...
    fn needs_capacity(&self) -> bool {
        ...
        let load_factor = self.entries.len() as f64 / self.capacity() as f64;
        load_factor > LOAD_FACTOR
    }

    /// Allocates more capacity and updates `indices` based on the current load factor **only if necessary**.
    fn maybe_allocate(&mut self) {
        if self.capacity() == 0 {
            self.expand_capacity(1);
        } else if self.needs_capacity() {
            // Update the capacity
            self.expand_capacity(self.additional_capacity());
    ...
    /// Reserves capacity for at least `additional` more elements to be inserted in the `OmniMap`.
    /// The resulting capacity will be equal to `self.capacity() + additional`.
    ///
    /// # Time Complexity
    ...
    pub fn reserve(&mut self, additional: usize) {
        ...
        self.expand_capacity(additional);
        // Rehash the entries into the new resized indices
        self.update_indices();
    }
    ...
    pub fn upsert(&mut self, key: K, value: V) {
        // Allocate capacity if necessary
        self.maybe_allocate();
        ...
        if let Some(existing_index) = index.iter().find(|&&i| self.entries[i].0 == key) {
    ...
    pub fn memory_usage(&self) -> usize {
        // Memory used by the entries vector, including the Vec overhead
        let entries_memory: usize = size_of::<Vec<(K, V)>>()
            + self
                .entries
                .iter()
        ...
        // Memory used by the indices vector, including the Vec and VecDeque overhead
        let indices_memory: usize = size_of::<Vec<VecDeque<usize>>>()
            + self
                .indices
                .iter()
                .map(|vec_deque| {
